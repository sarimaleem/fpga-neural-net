...
string classifyImage(const string& imagePath, bool dbg=false, double param=0.057, int sThresh=5, int rThresh=4, int pThresh=3) {
    ...
    // Draw contours
    drawContours(hand, contours, -1, Scalar(0, 255, 0), 2); // You can adjust the color and thickness here
    if (dbg) imshow("Hand", hand);
    ...
    for (const auto& point : approx) {
        circle(hand, point, 5, Scalar(0, 0, 255), -1); // Draw a red circle at each vertex
    }
    if (dbg) imshow("Hand", hand);
